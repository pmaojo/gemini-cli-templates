...
- [ ] **Repo Cleanup:** Remove Claude-specific legacy files (`.claude/`, `CLAUDE.md` in root) to clear confusion.
...
- [ ] **Mass Text Replacement (Repo-wide):**
    - Run a script to replace "Claude Code" with "Gemini CLI" in all documentation and comments.
    - Replace "Anthropic" with "Google" where appropriate in docs.
...
- [ ] **Create Migration Script (`scripts/convert_templates_to_gemini.py`):**
    ...
    - **Agents:** Read markdown, regex replace "You are Claude" -> "You are Gemini", "Anthropic" -> "Google". Save in place.
...
- [ ] **Frontend Rebranding (`cli-tool/src/analytics-web/`):**
    ...
    - Update "Claude State" components to say "Gemini State".
...
- [ ] **Final Polish:** Check `CONTRIBUTING.md` and `CODE_OF_CONDUCT.md` for stray "Claude" references.
