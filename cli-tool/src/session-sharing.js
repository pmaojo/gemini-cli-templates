...
/**
 * SessionSharing - Handles exporting Gemini CLI sessions as downloadable context
 */
class SessionSharing {
  ...
  async exportSessionAsMarkdown(conversationId, conversationData, options = {}) {
    ...
    try {
      ...
      const filename = `gemini-context-${projectName}-${date}.md`;
  ...
  /**
   * Convert conversation messages to markdown format optimized for Gemini CLI
  ...
  convertToMarkdown(messages, conversationData, stats) {
    ...
    // Header for Gemini CLI
    lines.push('# Previous Conversation Context\n');
    lines.push('> **Note to Gemini CLI**: This file contains the complete conversation history from a previous session. Read and understand this context to continue helping the user with their task.\n');
    ...
    lines.push('\n---');
    lines.push('');
    lines.push('*Generated by Gemini CLI Templates - [gemini-cli-templates.vercel.app-templatesgemini-cli-templates.vercel.appp](https://gemini-cli-templates.vercel.app)*');
  ...
  async exportSessionData(conversationId, conversationData, options = {}) {
    ...
    // Convert parsed messages back to JSONL format (original Gemini CLI format)
    ...
    const exportData = {
      ...
      conversation: {
        ...
        model: conversationData.modelInfo?.primaryModel || 'gemini-gemini-2.0-flash-4-5-20250929'
      },
      ...
      metadata: {
        ...
        description: 'Gemini CLI session export - can be cloned with: npx gemini-cli-templates@latest --clone-session <url>'
      }
    };
   ...
   * Downloads the session and places it in the correct Gemini CLI location
  ...
  async cloneSession(url, options = {}) {
    ...
    try {
      ...
      // 3. Install session in Gemini CLI directory
      ...
      const resumeCommand = `gemini --resume ${installResult.conversationId}`;
      console.log(chalk.yellow(`\n💡 To continue this conversation, run:`));
      console.log(chalk.white(`\n   ${resumeCommand}\n`));
      console.log(chalk.gray(`   Or open Gemini CLI to see it in your sessions list`));
  ...
  async downloadSession(url) {
    try {
      ...
      if (error.message.includes('Unexpected token')) {
        throw new Error('Invalid session file - corrupted or not a Gemini CLI session');
      }
      throw error;
    }
  }
  ...
  /**
   * Install session in Gemini CLI directory structure
  ...
  async installSession(sessionData, options = {}) {
    ...
    // Format: ~/.gemini/projects/-path-to-project/
    const projectDir = path.join(geminiDir, 'projects', projectDirName);
