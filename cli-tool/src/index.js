...
async function installIndividualCommand(commandName, targetDir, options) {
  ...
  try {
    ...
    if (commandName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/pmaojo/Gemini-cli-templates/main/cli-tool/components/commands/${commandName}.md`;
    } else {
...
async function getAvailableAgentsFromGitHub() {
  try {
    ...
    // Fallback to gemini-cli-templates.vercel.app API if local file not found
    try {
      // Try gemini-cli-templates.vercel.app API first
      const apiResponse = await fetch('https://gemini-cli-templates.vercel.app/api/agents.json');
      if (apiResponse.ok) {
        const apiData = await apiResponse.json();
        
        if (apiData.agents && Array.isArray(apiData.agents)) {
          console.log(chalk.green(`✅ Loaded ${apiData.agents.length} agents from gemini-cli-templates.vercel.app API`));
          return apiData.agents;
        }
      }
    } catch (apiError) {
      console.warn('Could not fetch from gemini-cli-templates.vercel.app, trying GitHub API...');
    }
    
    // If gemini-cli-templates.vercel.app API fails, try GitHub API as secondary fallback
...
async function installIndividualSkill(skillName, targetDir, options) {
  ...
  try {
    ...
    const githubApiUrl = `https://api.github.com/repos/pmaojo/Gemini-cli-templates/contents/cli-tool/components/skills/${skillName}`;
...
function generateWorkflowYAML(workflowData) {
  const yaml = `#═══════════════════════════════════════════════════════════════════════════════
# 🤖 GEMINI CODE WORKFLOW CONFIGURATION
#═══════════════════════════════════════════════════════════════════════════════
#
...
${workflowData.steps.map((step, index) => `  - step: ${index + 1}
    ...
    # GEMINI CODE INSTRUCTIONS FOR THIS STEP:
    gemini_instructions: |
    ...
    action_template: |
      ...
      # [GEMINI CODE WILL REPLACE THIS WITH ACTUAL EXECUTION]
...
#═══════════════════════════════════════════════════════════════════════════════
# 🤖 GEMINI CODE INTEGRATION SETTINGS
#═══════════════════════════════════════════════════════════════════════════════
...
geminicode:
  ...
  # WORKFLOW EXECUTION INSTRUCTIONS FOR GEMINI:
  ...
  try {
    ...
    try {
      ...
      if (await fs.pathExists(componentsDir)) {
        ...
        const baseUrl = 'https://raw.githubusercontent.com/pmaojo/Gemini-cli-templates/main/cli-tool/components/sandbox/e2b';
