...
class RedliningValidator:
    ...
    def validate(self):
        ...
        # First, check if there are any tracked changes by Gemini to validate
        try:
            ...
            # Check for w:del or w:ins tags authored by Gemini
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if self.verbose:
                print("PASSED - All changes by Gemini are properly tracked")
            return True

    def _generate_detailed_diff(self, original_text, modified_text):
        """Generate detailed word-level differences using git word diff."""
        error_parts = [
            "FAILED - Document text doesn't match after removing Gemini's tracked changes",
            "",
            "Likely causes:",
    ...
    def _remove_gemini_tracked_changes(self, root):
        ...
        for parent in root.iter():
            to_remove = []
            for child in parent:
                if child.tag == ins_tag and child.get(author_attr) == "Gemini":
                    to_remove.append(child)
            for elem in to_remove:
                parent.remove(elem)

        # Unwrap content in w:del elements where author is "Gemini"
        ...
        for parent in root.iter():
            to_process = []
            for child in parent:
                if child.tag == del_tag and child.get(author_attr) == "Gemini":
